...
public:
    ...
    session_start_steady_(std::chrono::steady_clock::now())
  {
    ...
    RCLCPP_INFO(
      get_logger(),
      "Keyboard ready: W/S pitch, A/D yaw, R/F thrust, Space stop, T auto-track toggle, Ctrl+C exit");
    if (log_enabled_) {
...
private:
  ...
  // [신규 추가] 의존성 없는 쿼터니언 수학 헬퍼
  struct Vector3 { double x, y, z; };
  Vector3 rotate_inverse(const Vector3& v, double qx, double qy, double qz, double qw) const
  {
    // 역 쿼터니언 (qw, -qx, -qy, -qz)를 이용한 벡터 회전
  ...
  void load_parameters()
  {
    ...
    // [신규 추가] 타겟 오도메트리 토픽
    target_odometry_topic_ = declare_parameter<std::string>(
    ...
    // [2단계 신규 추가] 자동 추적 토글 키
    auto_track_key_ = single_key(
    ...
    // [2단계 신규 추가] P-제어 게인
    p_gain_pitch_ = declare_parameter<double>("p_gain_pitch", 0.8);
    p_gain_yaw_ = declare_parameter<double>("p_gain_yaw", 0.8);
  ...
  void validate_parameters() const
  {
    const double values[] = {
      ...
      fin_left_neutral_rad_, fin_right_neutral_rad_, p_gain_pitch_, p_gain_yaw_};
  ...
  void create_state_subscribers()
  {
    ...
    target_sub_ = create_subscription<nav_msgs::msg::Odometry>(
      target_odometry_topic_, rclcpp::SensorDataQoS(),
      [this](const nav_msgs::msg::Odometry::SharedPtr message) {
        target_.valid = true;
        target_.x = message->pose.pose.position.x;
        target_.y = message->pose.pose.position.y;
        target_.z = message->pose.pose.position.z;
      });
  ...
  void configure_logging()
  {
    ...
    // [수정됨] CSV 헤더 맨 뒤에 타겟 추적 관련 변수 5개 추가
    log_file_ <<
      ...
      "target_x,target_y,target_z,error_pitch_rad,error_yaw_rad\n";
  ...
  void update()
  {
    ...
    // [신규 추가] 1단계: 타겟 상대 좌표 변환 및 오차각 연산 로직
    if (odometry_.valid && target_.valid) {
      // World Frame 상대 벡터
      Vector3 v_world = {
        target_.x - odometry_.position_x,
        target_.y - odometry_.position_y,
        target_.z - odometry_.position_z
      };

      // Quaternion 역변환을 통한 Body Frame 상대 벡터 산출
      Vector3 v_body = rotate_inverse(
        v_world,
        odometry_.orientation_x,
        odometry_.orientation_y,
        odometry_.orientation_z,
        odometry_.orientation_w
      );

      // 1. 내부 제어 및 CSV 기록용 오차각 유지 (제어 입력용)
      current_yaw_error_rad_ = std::atan2(-v_body.x, v_body.y);
      current_pitch_error_rad_ = std::atan2(v_body.z, v_body.y);

      // 2. 타겟의 상대 위치 각도 산출 (터미널 출력용: 내 기수 기준)
      // 전방(+Y), 우측(+X), 상향(+Z) 기준
      double target_azimuth_deg = std::atan2(v_body.x, v_body.y) * 180.0 / M_PI;
      double target_elevation_deg = std::atan2(v_body.z, v_body.y) * 180.0 / M_PI;

      // [2단계 신규 추가] 3. 자동 추적 모드일 경우 P-제어기 적용
      if (auto_track_mode_) {
        // P-게인을 곱한 후 Fin 물리 한계 내로 안전하게 클램핑
        pitch_target_rad_ = limited_fin_command(current_pitch_error_rad_ * p_gain_pitch_);
        yaw_target_rad_ = -limited_fin_command(current_yaw_error_rad_ * p_gain_yaw_);
      }

      // ROS 표준 터미널 즉시 출력 (타이머 주기인 20Hz마다 출력됨)
      // 상태 표시줄처럼 보이도록 출력 내용에 자동 추적 상태를 덧붙임
      RCLCPP_INFO(
        get_logger(),
        "[Target Rel] Body_V(X:%+.2f, Y:%+.2f, Z:%+.2f) | Target: Yaw(Right+):%+.1f° Pitch(Up+):%+.1f° | Auto:%s",
        v_body.x, v_body.y, v_body.z,
        target_azimuth_deg,
        target_elevation_deg,
        auto_track_mode_ ? "ON" : "OFF"
      );
    }
  ...
  {
    // [2단계 신규 추가] 자동 추적 토글 로직
    if (key == auto_track_key_) {
      auto_track_mode_ = !auto_track_mode_;
      append_event(auto_track_mode_ ? "AUTO_ON" : "AUTO_OFF");
      RCLCPP_INFO(get_logger(), "Auto Track Mode: %s", auto_track_mode_ ? "ON" : "OFF");
      return; // 토글 시 다른 수동 입력은 무시
    }

    // 수동 제어 로직 (자동 추적 중에는 W/S/A/D 키 입력을 무시하여 충돌 방지)
    if (key == nose_push_key_ && !auto_track_mode_) {
      pitch_target_rad_ = limited_fin_command(pitch_target_rad_ + fin_command_rad_);
      append_event("W");
      print_pitch_target();
    } else if (key == nose_pull_key_ && !auto_track_mode_) {
      pitch_target_rad_ = limited_fin_command(pitch_target_rad_ - fin_command_rad_);
      append_event("S");
      print_pitch_target();
    } else if (key == yaw_left_key_ && !auto_track_mode_) {
      yaw_target_rad_ = limited_fin_command(yaw_target_rad_ + fin_command_rad_);
      append_event("A");
      print_yaw_target();
    } else if (key == yaw_right_key_ && !auto_track_mode_) {
      yaw_target_rad_ = limited_fin_command(yaw_target_rad_ - fin_command_rad_);
      append_event("D");
      print_yaw_target();
    } else if (key == thrust_increase_key_) { // 추력은 자동/수동 상관없이 제어 가능
  ...
  void write_log_row(const CommandSnapshot & commands)
  {
    ...
    log_file_ << wall_time << ',' << elapsed << ',' << now().seconds() << ',' << events << ',' <<
      ...
      current_pitch_error_rad_ << ',' << current_yaw_error_rad_ << '\n';
  ...
  // [2단계 신규 추가]
  char auto_track_key_{};
  bool auto_track_mode_{false};
  double p_gain_pitch_{0.8};
  double p_gain_yaw_{0.8};
  ...
  double current_pitch_error_rad_{0.0};
  double current_yaw_error_rad_{0.0};
